      ...
      <div class="row">
        <div class="col-md-6 mx-auto">
          ...
          <form v-on:submit.prevent="addNewService">	  
				...
				<button v-if="this.isEdit == false" type="submit" style="width: 100%;background: #4f52c8" class="btn btn-success btn-block  mt-3" >Добавить</button>
				<button v-else type="button" v-on:click="update()" style="width: 100%;background: #4f52c8"class="btn btn-primary btn-block  mt-3" >Обновить</button>
          </form>
          ...
          <table class="table">
            <tr v-for="(todo) in todos" v-bind:key="todo.id" v-bind:title="todo.name">
              ...
              <td class="text-right">
                <button v-on:click="editCl(todo.id_client,todo.date, todo.number,todo.count,todo.id)" style="background: #a4b0bb;color: #eaf5ff;" class="btn">Редактировать</button>
                <button v-on:click="deleteCl(todo.id)" style="background: #ea6975;color: #eaf5ff;margin-top:11px" class="btn">Удалить</button>
              </td>
            </tr>
          </table>
        </div>
