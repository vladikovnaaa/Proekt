		...
		____________________________________________________________</p>
          <table class="table">
            <tr v-for="(todo) in todos" v-bind:key="todo.id" v-bind:title="todo.name">
              ...
              <td class="text-right">
                <button v-on:click="editCl(todo.name,todo.score, todo.city,todo.telephone,todo.id)"  style="background: #a4b0bb;color: #eaf5ff;" class=" btn">Редактировать</button>
                <button v-on:click="deleteCl(todo.id)" style="background: #ea6975;color: #eaf5ff;" class="btn">Удалить</button>
              </td>
            </tr>
          </table>
        </div>
