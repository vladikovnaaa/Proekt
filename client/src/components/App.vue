		...
		____________________________________________________________</p>
          <table class="table">
            <tr v-for="(todo) in todos" v-bind:key="todo.id" v-bind:title="todo.model">
              ...
              <td class="text-right">
                <button v-on:click="editCl(todo.id,todo.model, todo.description, todo.year, todo.sum)" style="background: #a4b0bb;color: #eaf5ff;" class="btn">Редактировать</button>
                <button v-on:click="deleteCl(todo.id)"  class="btn" style="background: #ea6975;color: #eaf5ff;margin-top:11px">Удалить</button>
              </td>
            </tr>
          </table>
        </div>
